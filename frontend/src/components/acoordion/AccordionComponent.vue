<script setup lang="ts">
  ...
  import ModalComponent from '../common/modal/ModalComponent.vue';
  ...
  const deleteAccordionItem = (index: number, id: number) => {
...
<template>
  <div>
    <div class="mt-6 bg-[#fff] w-[980px] m-auto flex flex-col p-6 shadow-md rounded-md" v-for="(item, index) in props.items" :key="item.id">
      <div class="flex justify-between">
        ...
        <div class="flex flex-row items-center">
          ...
          <ModalComponent v-if="visibleModals[item.id]">
            <div class="mb-6">
              <h1 class="text-[#f96555] text-4xl mb-6 text-center">Look out!</h1>
              <p class="text-[#363636] text-2xl text-center">Are you sure you want to remove {{ item.title }}?</p>
            </div>
            <div class="flex flex-col">
              <button class="mb-6 leading-normal font-bold rounded-[24px] border-solid border-2 border-[#128cf4] text-[#128cf4] pt-4 pb-4 mr-3" @click="toggleModal(item.id)" type="button">No, don't delete it</button>
              <button class="leading-normal font-bold rounded-[24px] border-solid border-2 bg-[#f96555] text-[#ffffff] pt-4 pb-4" @click="deleteAccordionItem(index, item.id)" type="button">Yes, delete</button>
            </div>
          </ModalComponent>
        </div>
        </div>
